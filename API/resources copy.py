...
async def get_project_capacity_allocation(request: Request, project_id: int):
    ...
    for period_key in sorted(aggregated_data.keys()):
        ...
        for resource_detail in interval_data.get('resource_details', []):
            ...
            # Add cumulative fields to the resource detail
            updated_detail = dict(resource_detail)
            updated_detail.update(resource_cumulatives[resource_id])
...
async def resource_capacity_allocation_per_portfolio(
    ...
    async def fetch_resource(resource_id):
        return await get_resource_capacity_allocation_route(
    ...
    for rid, resp in zip(resource_ids, responses):
        ...
        if isinstance(response_data, dict) and "resource_details" in response_data:
            resource_details_map[rid] = response_data["resource_details"]
        ...
        for period in data:
            ...
            resource_detail = resource_details_map.get(rid, {})
            resource_period_data = {
                "resource_id": rid,
                "resource_name": resource_detail.get("resource_name", f"Resource {rid}"),
